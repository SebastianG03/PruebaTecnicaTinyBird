...
class MetricsEntry(BaseModel):
    country: Optional[str] = Field(
        description="Código ISO del país", examples=["CL", "AR", "BR"]
    )
    from_date: Optional[str] = Field(
        description="Fecha inicial", examples=["2022-01-01T00:00:00Z"]
    )
    to_date: Optional[str] = Field(
        description="Fecha final", examples=["2022-01-01T00:00:00Z"]
    )
    ...
    def validate_country(cls, value: str) -> str:
        if value.strip():
            try:
                ...
                raise HTTPException(
                    detail="El código de país no es válido", status_code=400
                )
    ...
    def validate_from_date(cls, value: str, info) -> str:
        if value.strip():
            try:
                ...
                raise HTTPException(
                    detail="El formato de la fecha es incorrecto", status_code=400
                )
            ...
            if to_date and datetime.strptime(
                value, "%Y-%m-%dT%H:%M:%SZ"
            ) > datetime.strptime(to_date, "%Y-%m-%dT%H:%M:%SZ"):
                raise HTTPException(
    ...
    def validate_to_date(cls, value: str, info) -> str:
        ...
        if value.strip():
            ...
            if from_date and datetime.strptime(
                value, "%Y-%m-%dT%H:%M:%SZ"
